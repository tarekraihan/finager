...
$(function(event) {
		...
		$('.inputWrapper input').blur(function(){
			...
			if(finAsst < 10000 && finAsst < amtDetail){
				alert("Minimum amount should be 10000 & maximum amount 10000000");
				$("#finalAssest").val("10000");
				$(this).parents('.slideWrapper').find('.draggable, .highLight ').css('left',0);
				$(this).parents('.slideWrapper').find('ul').animate({marginLeft:'0px'});
			}
			else if(finAsst >=10000 && finAsst <=amtDetail)
			{
				...
				if(valInput == amtDetail)
				{
					...
					$(this).parents('.slideWrapper').find('ul').animate({marginLeft:-6785});	
				...
				{
					var marLft_input =  ((valInput/diff)-1) *35;
					$(this).parents('.slideWrapper').find('ul').animate({marginLeft:-6745});	
					$('#alreadySaved').find('.draggable').animate({left:175});	
				}

				else if(valInput == 9900000)
				{
					var marLft_input =  ((valInput/diff)-1) *35;
					$(this).parents('.slideWrapper').find('ul').animate({marginLeft:-6710});	
					$('#alreadySaved').find('.draggable').animate({left:175});	
				}

				else if(valInput == 9850000)
				{
					var marLft_input =  ((valInput/diff)-1) *35;
					$(this).parents('.slideWrapper').find('ul').animate({marginLeft:-6685});	
					$('#alreadySaved').find('.draggable').animate({left:175});	
				}

				else if(valInput == 9800000)
				{
					var marLft_input =  ((valInput/diff)-1) *35;
					$(this).parents('.slideWrapper').find('ul').animate({marginLeft:-6650});	
					$('#alreadySaved').find('.draggable').animate({left:175});	
				}
			...
			else if(finAsst <0 || finAsst >=amtDetail )
			{
				...
				alert("Minimum amount should be 10000 & Maximum amount should be 10000000");
				$("#finalAssest").val("10000");
				$(this).parents('.slideWrapper').find('.draggable, .highLight ').css('left',0);
				/*$(this).val(amtDetail);
...
/* Changes Done on 03-06-2014 END */
		...
		if($(this).find('label').text()=="Home Loan Calculator")
		{
			...
			$("#existingLifeCover ul, #finacialAssest ul, #finacialAssest1 ul").html(valLife + cr_1_life );
				/*for(var j =0; j<4; j++)
				{
					...
					newWid = len *wid;
					$('.calcSection').eq(j).find('ul').css('width', '3000px');	
				}*/
				
				wid = $('.calcSection').eq(0).find('ul li').width();
				len = $('.calcSection').eq(0).find('ul li').length;
				newWid = len *wid;
				$('.calcSection').eq(0).find('ul').css('width', '3000px');	
				
				wid = $('.calcSection').eq(3).find('ul li').width();
				len = $('.calcSection').eq(3).find('ul li').length;
				newWid = len *wid;
				$('#liability').children('ul').css('width', '180px');
		}
		else if($(this).find('label').text()=="Personal Loan Calculator")
		{
			var valLife='', cr_1_life='';
			...
			*/
			...
			len = $('.calcSection').eq(0).find('ul li').length;
			newWid = len *wid;
			$('.calcSection').eq(0).find('ul').css('width', 7210);	
	...
	$( ".draggable" ).mouseup(function(){
		...
		var ulMar = thMar + 35 + Math.abs(parseInt($(this).parents('.dragBox').find('.slideImg ul').css('margin-left')));
		...
		if( ulMar >= ulWid ){
			...
			{
				$(this).parents('.calcSection').find('.draggable, .highLight ').css('left',150);
			}
			else
			{
				$(this).parents('.calcSection').find('.draggable, .highLight ').css('left',150);
